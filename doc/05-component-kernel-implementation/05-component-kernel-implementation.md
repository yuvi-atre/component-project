...
For representation I will choose a Map where the key is a String representing the name of the item and the value is an integer representing the quantity of the item. I'm choosing a map because it is efficient and just easy to use for the implementation that I'm looking for. For convention the key will always be a valid item name non-null and non-empty string. The value will always be a non-negative, non-zero number.
